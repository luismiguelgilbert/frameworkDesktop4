<template>
    <q-card v-if="senderData" style="min-width: 800px;">
        ...
        <q-card-actions align="around">
            ...
            <q-btn color="primary" label="Descargar" icon="fas fa-download" @click="downloadMail" :loading="isDownloadingEML">
                <template v-slot:loading>Descargando...</template>
            </q-btn>
        </q-card-actions>
...
export default({
  ...
  data () {
    return {
      ...
      emlFile: null, isDownloadingEML: false
      //selectedRows: [],
    }
  },
  mounted(){
  ...
  methods:{
    ...
    downloadMail(){
        this.isBusy = true;
        this.$axios.post( this.apiURL + 'generateEMLMail', {
