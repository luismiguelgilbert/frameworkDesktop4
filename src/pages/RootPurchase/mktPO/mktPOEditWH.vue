    ...
    <q-card style="width: 100%; max-width: 200px; height: 150px;">
      <q-toolbar class="bg-primary text-white">
        <q-item>
          ...
          <q-item-section>
            <q-item-label>Facturados</q-item-label>
          </q-item-section>
        </q-item>
      </q-toolbar>
      <q-card-section class="absolute-center text-primary q-mt-lg" style="fontSize: 100px; ">
        {{lines.filter(x=>x.quantityInvoiced>0).length}}
      </q-card-section>
    </q-card>
  </div>
    ...
    <q-item v-for="row in lines" :key="row.lineID" clickable>
      ...
      <q-item-section>
        <q-stepper :value="0" flat ref="stepper" color="primary" animated class="no-padding" >
          ...
          <q-step :name="4"
            :title="$q.screen.gt.sm?'Facturado':undefined" 
            :caption="$q.screen.gt.sm?(row.quantityInvoiced):undefined" 
            icon="fas fa-file-invoice-dollar" done-icon="fas fa-invoice-dollar"
            :done="row.quantityInvoiced>0" />
