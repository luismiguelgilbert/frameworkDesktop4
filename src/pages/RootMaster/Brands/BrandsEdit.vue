...
export default ({
  ...
  data () {
    return {
        moduleName: "Brands", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spInvMasterBrandsSelectEdit',
            headers: { Authorization: "Bearer " + this.$q.sessionStorage.getItem('jwtToken') },
            params: {
                userCode: this.userCode,
                userCompany: this.userCompany,
                userLanguage: 'es',
                row_id: this.editRecord&&this.editRecord.row&&this.editRecord.row.brandID_ux?this.editRecord.row.brandID_ux:0,
    ...
    saveData(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spInvMasterBrandsUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
                        //"sys_user_language": this.$q.sessionStorage.getItem('sys_user_language'),
                        row_id: this.editMode?0:this.editRecord.row.brandID_ux,
