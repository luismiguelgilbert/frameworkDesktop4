...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nueva Clasificación':'Editar Clasificación: '+editRecord.value}}</q-toolbar-title>
            <q-space />
...
import basicComponent from './InvTypesEditBasic'
import historyComponent from './InvTypesEditHistory'
...
export default ({
  ...
  data () {
    return {
        moduleName: "InvTypes", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spinvMasterTypesSelectEdit',
    ...
    saveData(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spinvMasterTypesUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
