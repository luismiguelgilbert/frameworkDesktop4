...
>
    <template v-slot:top>
        <div :class="'text-h6 '+(userColor=='blackDark'?'ellipsis text-white':'ellipsis text-primary')" style="max-width: 70%;" >Marque los usuarios a quienes desee asignar acceso a esta compañía ({{users.filter(x=>x.is_allowed).length}})</div>
        <q-space />
...
export default ({
    data () {
        return {
            moduleName: "Companies", filterString: ''
        }
    },
    methods:{
