...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nuevo Módulo':'Editar Módulo: '+editRecord.value}}</q-toolbar-title>
            <q-space />
            ...
            <template v-slot:before>
                <q-list>
                    <q-item clickable @click="tab='basic'" :active="tab=='basic'" active-class="bg-primary text-white" >
                        <q-item-section side>
                            <q-icon name="fas fa-database" :color="tab=='basic'?'white':'grey-7'" />
                        </q-item-section>
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='basic'?'text-white':'text-grey-7')">Información del Módulo</q-item-label>
                        </q-item-section>
                    </q-item>
                    <q-item clickable @click="tab='columns'" :active="tab=='columns'" active-class="bg-primary text-white" >
                        <q-item-section side>
                            <q-icon name="fas fa-columns" :color="tab=='columns'?'white':'grey-7'" />
                        </q-item-section>
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='columns'?'text-white':'text-grey-7')">Columnas</q-item-label>
                        </q-item-section>
                    </q-item>
                    <q-item clickable @click="tab='filters'" :active="tab=='filters'" active-class="bg-primary text-white" >
                        <q-item-section side>
                            <q-icon name="fas fa-filter" :color="tab=='filters'?'white':'grey-7'" />
                        </q-item-section>
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='filters'?'text-white':'text-grey-7')">Filtros de Sistema</q-item-label>
                        </q-item-section>
                    </q-item>
                    <q-item clickable @click="tab='users'" :active="tab=='users'" active-class="bg-primary text-white" >
                        ...
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='users'?'text-white':'text-grey-7')">Filtros de Usuarios</q-item-label>
                        </q-item-section>
                    </q-item>
...
import basicComponent from './ModulesEditBasic'
...
export default ({
  ...
  data () {
    return {
        moduleName: "Modules", router: this.$router,
