    ...
    methods:{
        onSelectionChanged() {
            ...
            this.$refs['mainTreeList'].instance.getSelectedRowKeys('all').map(x=>{
                if(!(new_ticked_links.some(y=>y.sys_link_id==x))){
                    new_ticked_links.push({sys_link_id: x})
                }
            }); 
            this.ticked_links = new_ticked_links
        },
    },
    computed:{
        ...
        lookup_links: {
            get () {  return this.$store.state[this.moduleName].editData.lookup_links }
        },
        ticked_links: {
            get () { return this.$store.state[this.moduleName].editData.ticked_links },
            set (val) { this.$store.commit((this.moduleName)+'/updateEditDataAttribute', {key: 'ticked_links', value: val}) }
        },
