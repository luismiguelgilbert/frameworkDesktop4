<template>
    <div style="margin: -16px;">
        <DxTreeList
            ...
            :data-source="lookup_links_mobile"
            :show-borders="false"
            ...
            <DxColumn data-field="label" caption="Módulo" />
...
export default ({
    ...
    mounted(){
        ...
        this.ticked_links_mobile.map(x=>{
            if(x.sys_link_id){ 
                newSelectedRowKeys.push(x.sys_link_id);
            }else{
    ...
    methods:{
        onSelectionChanged() {
            ...
            this.$refs['mainTreeList'].instance.getSelectedRowKeys('all').map(x=>{
                if(!(new_ticked_links_mobile.some(y=>y.sys_link_id==x))){
                    new_ticked_links_mobile.push({sys_link_id: x})
                }
            }); 
            this.ticked_links_mobile = new_ticked_links_mobile
        },
    },
    computed:{
        ...
        lookup_links_mobile: {
            get () {  return this.$store.state[this.moduleName].editData.lookup_links_mobile }
        },
        ticked_links_mobile: {
            get () { return this.$store.state[this.moduleName].editData.ticked_links_mobile },
            set (val) { this.$store.commit((this.moduleName)+'/updateEditDataAttribute', {key: 'ticked_links_mobile', value: val}) }
        },
