...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nuevo rol':'Editar Rol: '+editRecord.value}}</q-toolbar-title>
            <q-space />
                ...
                <q-list>
                    <q-item clickable @click="tab='basic'" :active="tab=='basic'" active-class="bg-primary text-white" >
                        ...
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='basic'?'text-white':'text-grey-7')">Información del Rol</q-item-label>
                        </q-item-section>
                    </q-item>
                    <q-item clickable @click="tab='system'" :active="tab=='system'" active-class="bg-primary text-white" >
                        <q-item-section side>
                            <q-icon name="fas fa-desktop" :color="tab=='system'?'white':'grey-7'" />
                        </q-item-section>
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='system'?'text-white':'text-grey-7')">Permisos (Desktop)</q-item-label>
                        </q-item-section>
                    </q-item>
                    <q-item clickable @click="tab='mobile'" :active="tab=='mobile'" active-class="bg-primary text-white" >
                        <q-item-section side>
                            <q-icon name="fas fa-mobile-alt" :color="tab=='mobile'?'white':'grey-7'" />
                        </q-item-section>
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='mobile'?'text-white':'text-grey-7')">Permisos (Mobile)</q-item-label>
                        </q-item-section>
                    </q-item>
...
import basicComponent from './RolesEditBasic'
...
export default ({
  components:{
    ...
    ,linksmobileComponent: linksmobileComponent
    ,historyComponent: historyComponent
  },
  data () {
    return {
        moduleName: "Roles", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spSysProfilesSelectEdit',
            headers: { Authorization: "Bearer " + this.$q.sessionStorage.getItem('jwtToken') },
            params: { 
                userCode: this.userCode,
                userLanguage: 'es',
                userCompany: this.userCompany,
                row_id: this.editRecord&&this.editRecord.row&&this.editRecord.row.sys_profile_id_ux?this.editRecord.row.sys_profile_id_ux:0,
    ...
    saveDataExec(){
        this.$q.dialog({
                ...
                let newEditData = {
                    ...
                    ,ticked_links: this.editData.ticked_links.map(x=>x.sys_link_id)
                    ,ticked_links_mobile: this.editData.ticked_links_mobile.map(x=>x.sys_link_id)
                    ,users: this.editData.users.filter(x=>x.is_profile).map(x=>x.sys_user_code)
                }
                //console.dir(this.editData)
                //console.dir(newEditData)
                this.$axios.post( this.apiURL + 'spSysProfilesUpdate', {
                    ...
                    "currentRow": this.editMode?0:this.editRecord.row.sys_profile_id_ux,
