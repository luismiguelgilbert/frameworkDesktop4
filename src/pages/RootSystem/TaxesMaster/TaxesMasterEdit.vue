...
export default ({
  ...
  data () {
    return {
        moduleName: "TaxesMaster", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spSysTaxesSelectEdit',
    ...
    saveDataExec(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spSysTaxesUpdate', {
                    "userCode": this.$q.sessionStorage.getItem('sys_user_code'),
                    //"userLanguage": this.$q.sessionStorage.getItem('sys_user_language'),
                    "currentRow": this.editMode?0:this.editRecord.row.taxID_ux,
