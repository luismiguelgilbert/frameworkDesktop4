...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nuevo Impuesto':'Editar Impuesto: '+editRecord.value}}</q-toolbar-title>
            <q-space />
...
import basicComponent from './TaxesEditBasic'
import historyComponent from './TaxesEditHistory'
...
export default ({
  ...
  data () {
    return {
        moduleName: "Taxes", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spAccTaxesSelectEdit',
            headers: { Authorization: "Bearer " + this.$q.sessionStorage.getItem('jwtToken') },
            params: {
                userCode: this.userCode,
                userCompany: this.userCompany,
                userLanguage: 'es',
                row_id: this.editRecord&&this.editRecord.row&&this.editRecord.row.taxID_ux?this.editRecord.row.taxID_ux:0,
    ...
    saveData(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spAccTaxesUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
                        //"sys_user_language": this.$q.sessionStorage.getItem('sys_user_language'),
                        row_id: this.editMode?0:this.editRecord.row.taxID_ux,
