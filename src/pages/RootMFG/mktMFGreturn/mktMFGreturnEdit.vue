...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nueva Devolución de Insumos':'Editar Devolución de Insumos: '+editRecord.value}}</q-toolbar-title>
            <q-space />
            ...
            <template v-slot:before>
                <q-list>
                    ...
                    <q-item clickable @click="tab='lines'" :active="tab=='lines'" active-class="bg-primary text-white" >
                        ...
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='lines'?'text-white':'text-grey-7')">Devolución de Insumos</q-item-label>
                        </q-item-section>
                    </q-item>
...
import basicComponent from './mktMFGreturnEditBasic'
import linesComponent from './mktMFGreturnEditLines'
import whComponent from './mktMFGreturnEditWH'
...
export default ({
  ...
  data () {
    return {
        moduleName: "mktMFGreturn", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spmktMFGRetSelectEdit',
    ...
    saveData(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spmktMFGRetUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
