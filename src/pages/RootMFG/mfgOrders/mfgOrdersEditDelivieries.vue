...
<div style="margin: -16px;">
    <q-table
        ...
        :columns="[
            ...
            { name: 'invID', required: true, label: 'Producto Terminado', align: 'left', field: row => row.invID, sortable: true },
            { name: 'orderID', required: true, label: 'Orden #', align: 'right', field: row => row.orderID, sortable: true },
            { name: 'qtyIN', required: true, label: 'Cantidad PT', align: 'right', field: row => row.qtyIN, sortable: true },
            { name: 'qtyINReal', required: true, label: 'Cantidad Real', align: 'right', field: row => row.qtyINReal, sortable: true },
        ]"
        >
        ...
        <template v-slot:bottom-row >
            <q-tr>
                ...
                <q-td class="text-right text-subtitle2 text-primary">
                    {{lookup_mmktPRD.reduce((total,item)=>{return total + item.qtyINReal}, 0)}}
                </q-td>
            </q-tr>
            <q-tr></q-tr>
        </template>
...
export default ({
    ...
    computed:{
        ...
        lookup_mmktPRD: {
            get () { return this.$store.state[this.moduleName].editData.lookup_mmktPRD },
        },
        lookup_items: {
