...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nueva Entrega de Producto Terminado':'Editar Entrega de Producto Terminado: '+editRecord.value}}</q-toolbar-title>
            <q-space />
                ...
                <q-list>
                    <q-item clickable @click="tab='basic'" :active="tab=='basic'" active-class="bg-primary text-white" >
                        ...
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='basic'?'text-white':'text-grey-7')">Información de Entrega</q-item-label>
                        </q-item-section>
                    </q-item>
                    <q-item clickable @click="tab='lines'" :active="tab=='lines'" active-class="bg-primary text-white" >
                        <q-item-section side>
                            <q-icon name="fas fa-file-invoice-dollar" :color="tab=='lines'?'white':'grey-7'" />
                        </q-item-section>
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='lines'?'text-white':'text-grey-7')">Producto Terminado</q-item-label>
                        </q-item-section>
                    </q-item>
...
import basicComponent from './mktPRDEditBasic'
import linesComponent from './mktPRDEditLines'
import whComponent from './mktPRDEditWH'
...
export default ({
  ...
  data () {
    return {
        moduleName: "mktPRD", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spMktPRDSelectEdit',
    ...
    saveData(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spMktPRDUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
