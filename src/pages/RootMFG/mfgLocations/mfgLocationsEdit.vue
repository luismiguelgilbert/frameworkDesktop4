...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nueva Línea de Producción':'Editar Línea de Producción: '+editRecord.value}}</q-toolbar-title>
            <q-space />
...
import basicComponent from './mfgLocationsEditBasic'
...
export default ({
  ...
  data () {
    return {
        moduleName: "mfgLocations", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spMfgLocationsSelectEdit',
    ...
    saveData(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spMfgLocationsUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
