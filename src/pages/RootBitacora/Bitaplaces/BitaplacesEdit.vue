...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nuevo Punto de Control':'Editar Punto de Control: '+editRecord.value}}</q-toolbar-title>
            <q-space />
            ...
            <template v-slot:after>
                <q-tab-panels
                    ...
                    <q-tab-panel name="users">
                        <usersComponent ref="usersComponent" />
                    </q-tab-panel>
...
import basicComponent from './BitaplacesEditBasic'
import pictureComponent from './BitaplacesEditPicture'
import usersComponent from './BitaplacesEditUsers'
import filesComponent from './BitaplacesEditFiles'
import historyComponent from './BitaplacesEditHistory'
...
export default ({
  ...
  data () {
    return {
        moduleName: "Bitaplaces", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spBitaplacesSelectEdit',
            headers: { Authorization: "Bearer " + this.$q.sessionStorage.getItem('jwtToken') },
            params: { 
                userCode: this.userCode,
                userCompany: this.userCompany,
                userLanguage: 'es',
                row_id: this.editRecord&&this.editRecord.row&&this.editRecord.row.placeID_ux?this.editRecord.row.placeID_ux:0,
    ...
    saveData(){
        this.$q.dialog({
                ...
                let newEditData = {
                    ...
                    ,recipients: this.editData.users.filter(x=>x.is_recipient).map(x=>x.sys_user_code)
                    ,files: this.editData.files.map(x=>x.attachID)
                    //,files: this.editData.files
                }
                //console.dir(this.editData)
                //console.dir(newEditData)
                this.$axios.post( this.apiURL + 'spBitaplacesUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
