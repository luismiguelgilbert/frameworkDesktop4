...
<q-form style="margin: -16px;" ref="formulario" greedy autofocus no-error-focus spellcheck="false" autocorrect="off" autocapitalize="off" class="q-gutter-sm q-pa-md">
    ...
    <q-input
        label="Comentarios al salir" placeholder="Ingrese comentarios sobre este viaje" filled
        type="textarea" 
        :readonly="record_id > 0"
        v-model="out_comments"
        />
    
    <q-input
        label="Kilometraje al retornar" placeholder="Kilometraje al retornar" filled
        v-if="record_id > 0" type="number" :min="out_km"
        v-model="in_km" 
        :autofocus="record_id > 0"
        >
        <template v-slot:prepend><q-icon name="fas fa-tachometer-alt" color="primary" /></template>
    </q-input>

    <q-input
        label="Comentarios al retornar" placeholder="Ingrese comentarios sobre el retorno del viaje" filled
        type="textarea" 
        v-if="record_id > 0"
        v-model="in_comments"
        />
...
export default ({
    ...
    computed:{
        ...
        in_comments: {
            get () { return this.$store.state[this.moduleName].editData.basic.in_comments }, 
            set (val) { this.$store.commit((this.moduleName)+'/updateEditData', {section: 'basic', key: 'in_comments', value: val}) }  
        },
        in_km: { 
            get () { return this.$store.state[this.moduleName].editData.basic.in_km }, 
            set (val) { this.$store.commit((this.moduleName)+'/updateEditData', {section: 'basic', key: 'in_km', value: val}) }  
        },
        lookup_cars: { 
