...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nueva Ronda':'Editar Ronda: '+editRecord.value}}</q-toolbar-title>
            <q-space />
                ...
                <q-list>
                    <q-item clickable @click="tab='basic'" :active="tab=='basic'" active-class="bg-primary text-white" >
                        ...
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='basic'?'text-white':'text-grey-7')">Información de la Ronda</q-item-label>
                        </q-item-section>
                    </q-item>
...
import basicComponent from './BitaRoundsEditBasic'
import historyComponent from './BitaRoundsEditHistory'
...
export default ({
  ...
  data () {
    return {
        moduleName: "BitaRounds", router: this.$router,
  ...
  methods:{
    ...
    loadData(){
        ...
        this.$axios({
            method: 'GET',
            url: this.apiURL + 'spBitaroundsSelectEdit', //este módulo maneja 1 parámetro atípico (placeID)
            headers: { Authorization: "Bearer " + this.$q.sessionStorage.getItem('jwtToken') },
            params: { 
                userCode: this.userCode,
                userCompany: this.userCompany,
                userLanguage: 'es',
                placeID: this.editRecord&&this.editRecord.row&&this.editRecord.row.placeID_ux_val?this.editRecord.row.placeID_ux_val:0,
                row_id: this.editRecord&&this.editRecord.row&&this.editRecord.row.roundID_ux?this.editRecord.row.roundID_ux:0,
    ...
    saveData(){
        this.$q.dialog({
                ...
                let newEditData = {
                    ...
                    //,coordinates: this.editData.coordinates
                    //,files: this.editData.files
                }
                //console.dir(this.editData)
                //console.dir(newEditData)
                this.$axios.post( this.apiURL + 'spBitaroundsUpdate', {
                        userCode: this.userCode,
                        userCompany: this.userCompany,
                        //"sys_user_language": this.$q.sessionStorage.getItem('sys_user_language'),
                        placeID: this.$store.state[this.moduleName].editData.basic.placeID,
                        row_id: this.editMode?0:this.editRecord.row.roundID_ux,
