...
<q-page style="min-height: 200px; height: calc(100vh - 50px); overflow-y: hidden;" class="q-pa-none">
    ...
    <q-card class="q-ma-md rounder-corners shadow-3" v-if="dataLoaded">
        <q-toolbar :class="'q-pr-none text-subtitle2 '+(userColor=='blackDark'?'text-white':'text-primary')">
            <q-toolbar-title class="text-weight-bolder">{{editMode?'Nueva Persona':'Editar Persona: '+editRecord.value}}</q-toolbar-title>
            <q-space />
                ...
                <q-list>
                    <q-item clickable @click="tab='basic'" :active="tab=='basic'" active-class="bg-primary text-white" >
                        ...
                        <q-item-section v-if="$q.screen.gt.xs">
                            <q-item-label :class="'text-subtitle2 '+(tab=='basic'?'text-white':'text-grey-7')">Datos de la Persona</q-item-label>
                        </q-item-section>
                    </q-item>
...
import basicComponent from './SchPersonasEditBasic'
import phonesComponent from './SchPersonasEditPhones'
import addressComponent from './SchPersonasEditAddress'
import mailsComponent from './SchPersonasEditMails'
import childrenComponent from './SchPersonasEditChildren'
import groupsComponent from './SchPersonasEditGroups'
import apostoladosComponent from './SchPersonasEditApostolados'
...
export default ({
  ...
  data () {
    return {
        moduleName: "SCHPersonas", router: this.$router,
    ...
    saveDataExec(){
        this.$q.dialog({
                ...
                this.$axios.post( this.apiURL + 'spSchPersonasUpdate', {
                    ...
                    "row_id": this.editMode?0:this.editRecord.row.sys_user_code_ux,
