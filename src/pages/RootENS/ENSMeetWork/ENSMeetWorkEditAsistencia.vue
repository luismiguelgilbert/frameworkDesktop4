...
<q-form style="margin: -16px;" ref="formulario" greedy autofocus no-error-focus spellcheck="false" autocomplete="off" autocorrect="off" autocapitalize="off" class="q-gutter-sm q-pa-md">
    ...
    <q-list  bordered separator >
        <q-item-label header class="text-primary">Quiénes asistieron a la Reunión de Pilotaje?</q-item-label>

        <q-item tag="label"  v-for="(persona, index) in ensPilotMeetPersonas" :key="index" >
...
export default ({
    ...
    methods: {
        changeRowState(persona){
            let nuevaAsistencia = JSON.parse(JSON.stringify(this.ensPilotMeetPersonas))
            ...
            this.ensPilotMeetPersonas = nuevaAsistencia
        },
    ...
    computed:{
        ...
        ensPilotMeetPersonas: {
            get () { return this.$store.state[this.moduleName].editData.ensPilotMeetPersonas },
            set (val) { this.$store.commit((this.moduleName)+'/updateEditDataAttribute', {key: 'ensPilotMeetPersonas', value: val}) }
        },
